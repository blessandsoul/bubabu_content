...
- `scene.lighting_advanced.color_temperature` — integer Kelvin 2000-10000 (Bubabu Pixar warmth: typically 3200-4600)
- `scene.lighting_advanced.type` — non-empty enum (cinematic for Pixar feature-film aesthetic)
- `technical.camera.model` — enum value (ARRI Alexa Mini OK for Pixar look)
- `technical.material_science.subsurface_scattering` — `true` (plush fur SSS mandatory)
- `advanced.negative_prompt` — **≥15 inline** including Bubabu drift-prevention bans (no glow on body, no brown ears, no open beak, no 3.jpeg heart-eyes variant, no second Bubabu, no scary owl, no dark menacing, no halo, no body radiance, no vertical god-rays, no peach-pink, no lavender) + UNIVERSAL BAN TAIL
...
- `technical_rendering.motion_blur_strength` + `dynamic_range` + `noise_reduction` — all 3
- `advanced.negative_prompt` — **≥15** including Bubabu drift bans + UNIVERSAL BAN TAIL
...
- `"subsurface_scattering": false` → MUST be `true` for plush fur shader
...
## Smart Gifts Pair Pipeline (DEFAULT WORKFLOW · LOCKED 2026-05-28)
...
- `facebook.md` + `tiktok.md` — social copy EN body + KA caption
- `meta.md` — variant declaration + §H seasonal opt-in + featured SKU id + **`paired_ad: [ad-slug]` pointer** + production notes

**Day N+1 — Ad** (Smart Gifts ad spec, lightweight commercial):
- `brief.md` — cartoon slug pointer + SKU id + retail zone (Hero/Gift-Sets/Educational/Impulse/Premium-Seasonal) + age tier + price ₾ + ONE key benefit line
- `image.md` — Nano Banana 2 prompt that REUSES cartoon Scene 7/8/9 composition. Three valid modes:
  - **Mode A (frame-rip):** literal scene reuse, neutral background swap removes child-room context — pure Bubabu+SKU shot.
  - **Mode B (re-render):** Nano Banana with `1.jpeg`+`2.jpeg` Bubabu ref + SKU product photo ref + cartoon scene composition described in prompt body. Same pose/mood-state as cartoon scenes 7-9.
  - **Mode C (cartoon-still):** if cartoon scene 7-9 already meets brand quality, screenshot at full resolution, no re-render. Cheapest mode.
- `video.md` — OPTIONAL 4-8 sec motion variant (img2vid from `image.md` output, Bubabu still no speech, mascot mood-state animates SKU interaction)
- `post.md` — caption (1 line benefit + price ₾ + 1-line CTA), EN/KA/RU multi-version if cross-channel
- `cover.md` — FB/IG thumbnail from same key frame
- `meta.md` — `paired_cartoon: [cartoon-slug]` pointer + zone + age tier + channel list (FB/IG/TT/bubabu.ge)
...
Words BANNED in cartoon voiceover.md + video.md + cover.md + facebook.md + tiktok.md when a screen-tech SKU is featured: `screen / экран / ეკრანი / tablet / პლანშეტი / phone / TV / display / LCD / LED / battery / charge / app / WiFi / Bluetooth / USB / USB-C / button / power on / power off / drone / რაცია / walkie-talkie / laptop / ლეპტოპი / camera / გადასაღები / pixel`.
