...
> **📜 BUBABU_SCRIPT_ENGINE v1.0 (KIDS GEORGIAN BODY VOICE — MANDATORY for /bubabu Georgian script body):** Read body `bible/BUBABU_SCRIPT_ENGINE.md` BEFORE writing any Bubabu Georgian `script.md` / `voiceover.md` body / `text.md`. Corpus base = 12 KA transcripts from 2 top Georgian kids YT channels (`@ძილისპირულიზღაპრები` + `@jirafijoze1`). Locks: TWO modes (SLEEP=rhymed verse / DAY=prose), 6 hook archetypes (H1-H6), 5-beat arc, SACRED closing ritual «იავ ნანა ნანინა, ტკბილი ძილი ნებისა» (8/8 sleep stories), warmth markers (`პატარა`/`ტკბილი`/`მეგობარო`), banned vocab (no fear/money/screens), 65 WPM SLEEP / 75 WPM DAY, sentence median 7 words. Pre-save grep gate §10. Source corpus + analysis: `agents/bubabu/source/analysis.md`. Distinct from ALEKSO_TEXT_ENGINE (adult Russian, 150 WPM, paradox hooks).
