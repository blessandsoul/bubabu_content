...
## ADV MODE — Bubabu Adventures (narrator-led cartoon series — LOCKED 2026-05-24 · v3)
...
**v3 (2026-05-24) revisions** per `agents/premortem/content/20260524_bubabu_adv_ep01/transcript.md` + EP01 production session user directives: cloud-tree canon (R3, replaces star-nest above-clouds — anti-heaven-iconography), wing-arc spark mechanic (R4, replaces chest-glow ignition), one churchkhela R5 (single GE marker in EP01 only), SKIN_SYSTEM split (R6, identity-lock + 3×3×3 variant flex + §H seasonal — EPILOGUE_BEAT made OPTIONAL not mandatory). Narrator audition gate (R1) + Lyria librosa verification (R2) gate Phase B render. **R7 dual-cut requirement DROPPED 2026-05-24** — was premortem sub-agent over-engineering, Bernard/Remedy ship one master per episode and Bubabu follows the same. One master, period.
...
| **Child** | NEW child per episode. No recurring face → no face-lock pain across episodes. Each child = different age (3-12) / look / room / scenario. Children part-of-frame, not lip-sync speakers (narrator carries everything). |
...
| **Length** | 60-90 sec long master (target 75s) + REQUIRED 25-sec scroll-cut (R7). |
| **Scene count** | 12-14 scenes hard on long master; 7-8 beats on scroll-cut. |
| **Episode structure** | HYBRID per ep — ADVENTURE setup + MYSTERY beat + LESSON resolution. ONE of three sparks earns the resolution via WING-ARC TRAIL (R4) — not chest-glow. End on a VISUAL LANDING (last on-screen event). EPILOGUE_BEAT cosmic callback is OPTIONAL per ep (R6 v3 2026-05-24 — was mandatory v2, downgraded after EP01 user directive «нахуй вообще panchline»). |
| **Language** | EN master voiceover (Phase 1) + KA draft (Phase 1 second pass) — user finalizes KA. SRT generation = Phase 2, never pre-gen. |
| **Visual style** | Pixar 3D × Ghibli softness × Ghibli watercolor texture for cloud-tree beats. Per-ep variant palette declared from SKIN_VARIANTS grid below. NEVER photoreal tokens (no Sony/Sigma/DSLR/ISO). NEVER gold-leaf / white-gold / peach-pink / lavender / pure-white-glow / vertical light-shafts. |
...
| **Channel** | Bubabu official + Axiom Smart. SKIN_IDENTITY (below) locked on EP01 → all later eps inherit identity verbatim, declare variant combo from grid. |
...
### SKIN_IDENTITY (hard lock — verbatim through EP02-N) — R6

These lock on EP01 and propagate verbatim to every later ADV episode. Story / child / variant combo change; identity does not.
...
- **Universal closing line:** EN "the first one. there will be others." / KA "პირველი. სხვებიც იქნებიან." (provisional EP01 — line carries through series)
...
**v3 lock (2026-05-24):** EPILOGUE_BEAT is **OPTIONAL per episode**, NOT a mandatory recurring beat. Per user directive «нахуй вообще panchline» (EP01 production session): the closing line «the first one. there will be others» and the separate 3-sec cosmic-callback scene were dropped from EP01 because the final on-screen event (Scene 11: Emi opens window, dawn rises around both) IS the visual landing — no additional vocal punchline or cosmic cutaway required.
...
**EP01 = SKIPS EPILOGUE_BEAT.** Scene 11 dawn IS the close.

The wonder motif final phrase moves INTO the closing story scene (Scene 11 in EP01) instead of a separate scene 12.

### Output structure (per episode) — R7 (dual-cut requirement)
...
├── voiceover.md          — EN master (single TTS-paste block) + KA draft + audition test gate
├── video.md              — 12-14 Veo img2vid blocks for 75-sec long master
├── video_scroll_cut.md   — 7-8 Veo blocks for 25-sec FB/TT scroll-cut (face at f1)
...
└── meta.md               — variant declaration + §H opt-in + premortem verdict (EP01 only) + production notes
```
...
|-------|------|------|--------|
| 1 | COLD OPEN | Wide of ancient cloud-tree at dusk, watercolor texture, hollow visible at top, horizontal side-rake light from foliage gap. Bubabu silhouette on moss branch. NO god-rays. | — |
...
| 7 | THE CHILD | Inside room — child awake, listening for something she can't name. R5 marker: one churchkhela visible through doorway in soft focus (EP01 only). | — |
| 8 | THE WORLD | Wide of room + visible doorway. R5 churchkhela still soft-focus. | — |
...
| 14 *(OPT)* | EPILOGUE_BEAT | OPTIONAL per R6 v3 — invoke only if final story scene doesn't carry the landing. Bubabu on sill looking up, single small star, motif plays final time. SKIP by default — let last story scene be the visual close. EP01 SKIPS this scene. | — |

### 7-8 beat scroll-cut template (25-sec FB/TT) — R7

| # | Beat | Time |
|---|------|------|
| S1 | FACE AT F1 — Bubabu's plush face fills frame on windowsill, one eye-blink. Brand recognition in 0.5s. | 0.0-2.5s |
| S2 | CHILD APPEARS — smash-cut to child at window, wonder face. | 2.5-4.0s |
| S3 | FLASHBACK MONTAGE — 3 quick stills: cloud-tree hollow → bowl of three motes → paper-glider boat tilting down. Motif only, no narration. | 4.0-7.0s |
| S4 | THE SPARK GESTURE — wing-arc trail beside Bubabu. | 7.0-10.0s |
| S5 | WONDER MEETS WONDER — eye-contact through glass. | 10.0-14.0s |
| S6 | WINDOW OPENS — wide of child opening window, dawn light. | 14.0-18.0s |
| S7 *(OPT)* | EPILOGUE_BEAT — Bubabu on sill, single star above. Narrator closing line. OPTIONAL per R6 v3 — SKIP by default. EP01 SKIPS; S6 extends to fill 14.0-25.0s holding the dawn landing. | OPT (was 18.0-25.0s) |

**Validator:** scroll-cut f1 must contain Bubabu's face OR a clear wing-action — NEVER cosmic abstract. NEVER omit S4 (the spark beat).
...
10. **Hard NO list:** screen / tablet / phone / TV / app / website / "AI" word in body / brand mentions / specific city or country name (single R5 exception: one churchkhela in scenes 7-8 of EP01 only).

### Self-check before save (ADV-specific, additive to Bubabu MANDATORY PRE-SAVE GREP GATE)

- [ ] origin_story.md v2 canon respected (cloud-tree hollow / paper-glider / guiding star / 3 sparks / chosen owl / no specific city beyond R5)
- [ ] Bubabu silent (zero Speech tags for Bubabu)
- [ ] Narrator = Charon Storytime preset (R1) — pace, breath-pause, whisper-drop locked
- [ ] 12-14 scenes on long master + 7-8 scroll-cut beats with face/action at f1 (R7)
...
- [ ] EP01 only: PREMORTEM GATE run (series-skin lock = irreversible) — Mode L, verdict recorded in meta.md
- [ ] Origin canon v2 re-approval signal from Archil obtained before EP01 ships
