---
version: 2.44
last_bumped: 2026-06-15
---
> **🎨 PROMPT_ENGINE:** Read `bible/PROMPT_ENGINE.meta.yaml` at session start; read body `bible/PROMPT_ENGINE.md` when trigger fires per `mandatory_for`. v3.2 DUAL SPEC-only: image SPEC JSON pastes directly into Nano Banana 2, video SPEC JSON into Veo 3.1, no COMPILED prose layer. 16 anti-locks, §2.1 maximal-detail mandate, camera/lens/film-stock tokens.
...
- `/adops` for ad OPERATIONS: diagnose / kill / scale / report a LIVE Meta FB-IG campaign (Ads Manager metrics, budget, CPL/ROAS), NOT creatives
...
### Ad work routing (creative vs operations)
- Make an ad ASSET (static image / carousel / FOOH / UGC video) -> `/ad`, `/fooh`, or `/ugc`.
- RUN a live Meta campaign (diagnose reach-but-no-sales, kill losers, scale winners, set targeting/budget, read CPL/ROAS, write the report) -> `/adops`. This is the diagnose/kill/scale/report loop; the artifacts that used to be hand-written in `agents/bubabu/ads/*/meta.md` now belong to it.
- `/adops` output for bubabu is brand content: write it DRAFT-FIRST into `agents/bubabu/draft/[YYYYMMDD]_[slug]/` (or the campaign's `agents/bubabu/ads/...` folder), NEVER `agents/adops/content/` and never straight to `agents/bubabu/content/`. Bubabu taboos (kid-safe, no Cyrillic in frames, warm + smart voice), BRAND_ENGINE no-public-prices, and SOCIAL_ENGINE English-always copy still apply.
...
- `scene.style_anchor` (NEW MANDATORY field per scene 2026-05-30, upgraded to QUALITY-RICH template 2026-06-01 per user feedback «покачественнее... мутно») - non-empty string. Required template: `"Ultra-detailed Pixar feature-film 3D final-render — Disney/Pixar studio polish grade, sharp clean geometry, crisp edge separation between subjects and background, high-frequency detail on plush fur weave and material textures, vivid saturated Candy Pop kid-toy palette (NOT muted, NOT washed out, NOT desaturated), bright clear studio-polish lighting with warm key + soft fill + gentle rim. Reference quality bar: Boo close-up sharpness from Monsters Inc + Carl Fredricksen warm-render quality from Up + Miguel character detail from Coco. Stylized toy-like proportions. NEVER muddy, NEVER soft-focused, NEVER cheap-3D, NEVER photoreal, NEVER live-action, NEVER cinematic film camera grade."` Pasted verbatim into every image SPEC. Skipping = VIOLATION. Generic «Pure Pixar 3D render... NOT photoreal» template (pre-2026-06-01) DEPRECATED - produced muddy/soft drift in NB2 output.
...
Call appropriate sub-agent (`/mascot` for chroma stingers, `/eden` for viral hook scripts, `/ad` for an ad creative, `/adops` for running or diagnosing a LIVE Meta campaign, etc.)
...
**Day N+1 - Ad** (Smart Gifts ad spec, lightweight commercial - 6 files):
...
- `voiceover.md` - **soft emotional product explainer in Charon Storytime preset** (same voice as cartoon - brand voice consistency). 3 cues, ~13-15 EN words per cue (~40 words total, ~17-19 sec spoken @ 140 wpm). Sells RESULT (child interaction / learning / emotional value) NOT TOOL (specs/features). **ZERO price mentions, ZERO CTA verbs in narration** (per `feedback_bubabu_ad_voiceover_no_price_just_why_toy_matters`) - those live ONLY in `post.md` + `cover.md` baked headline. Off-screen narrator path (VOICEOVER_ENGINE §6 editor overlay - silent Veo render + Gemini TTS composite, NO Speech: tag in `video.md`). FULL SCRIPT block paste-ready single TTS run. KA verbatim via user's `voiceover_geo.md` per `feedback_bubabu_no_ai_ka_verse_or_prose`.
- `video.md` - **3 scene-blocks dual SPEC cartoon-style** (single unified file - NO separate `image.md`, MERGED 2026-05-30 per user directive «у меня все подточено под формат картун там фото промпт и видео промпт»). Each scene = paired NB2 image SPEC + Veo 3.1 video SPEC. Canonical 3-scene reveal arc:
  ...
  - **Video SPECs = FOLEY ONLY in Veo output** (`sfx_cues` array only, NO Speech: tag, NO narration/lip-sync/speech mentions). Narrator audio composited in editor over silent Veo + foley per VOICEOVER_ENGINE §6 + §7.
...
- `meta.md` - `paired_cartoon: [cartoon-slug]` pointer + zone + age tier + channel list with per-platform cut plan (FB feed = Scene 3 static + post.md caption / IG Reels = all 3 montage + VO + foley + optional Lyria / TikTok = Scene 1 hook + Scene 2 + Scene 3 + VO / YT Shorts = montage + VO).

**Editor composite layer order (mandatory):** silent Veo video tracks → foley layer (from Veo `sfx_cues`) → Charon VO layer at -3 dB above foley peak / -22 dB below VO peak so foley still breathes through → optional Lyria 3 brand-bell loop layer at -22 dB under foley. Mix mastered at -14 LUFS for FB/IG/TT/YT broadcast standard.
...
# Universal pre-save grep gates 2026-05-30 (all pair-folder SPEC files — cartoon/video.md + cartoon/cover.md + ad/video.md + ad/cover.md)
...
└── [YYYYMMDD]_pair[NN]_[sku-slug]/                ← one pair = one SKU
    ...
    └── ad/                                         ← odd day N+1
        ├── brief.md        (cartoon ref + SKU + price + benefit + 3-scene arc overview)
        ├── voiceover.md    (Charon Storytime — 3 cues soft emotional product explainer, ZERO price/CTA in narration. Off-screen narrator composited in editor. Added 2026-05-30 per `feedback_bubabu_ad_voiceover_no_price_just_why_toy_matters`)
        ├── video.md        (3 scene-blocks dual SPEC cartoon-style: JOY-HOP REVEAL + THEMATIC PLAY + POSSESSION HOLD — each scene = paired NB2 image SPEC + Veo video SPEC. FOLEY ONLY in Veo, narrator composited in editor. MERGED former image.md into here 2026-05-30 per `feedback_bubabu_ad_video_md_cartoon_style_dual_spec`)
        ├── post.md         (caption + price + CTA — EN body per SOCIAL_ENGINE LAW 1)
        ├── cover.md        (FB/IG thumbnail from Scene 3 image render — baked-text Pixar candy-pop with `BUBABU SMART GIFTS` brand wordmark only. NO price, NO benefit subheadline — price lives ONLY in post.md per `feedback_bubabu_ad_voiceover_no_price_just_why_toy_matters` scope extension 2026-05-31)
        └── meta.md         (paired_cartoon: [cartoon-slug] pointer + zone + age + channels + composite layer order)
...
**SKU_REF rule (LOCKED 2026-05-30 · path corrected 2026-06-04):** at episode/pair-folder creation, COPY the SKU product photo into the folder root as **`sku_ref.jpg`** via Bash `cp` - and do this **FIRST, before writing any SPEC** (the photo is the sacred source of truth; never build the package on a missing placeholder, per `feedback_bubabu_secure_sku_photo_first`). **Source = `C:\Users\User\Desktop\BUBABU\New Products_ Images\`** (~57 files). NOT `Downloads\BUBABU\…` - that folder was deleted; the old Downloads path here was the cause of a shipped-without-photo episode (user: «опять продукт без фото сделал»). **Filenames are OPAQUE:** `IMG_34xx.jpg` = the NEW-pipeline scan (mapped to SKUs by filename in `product_catalog.md`); `Copy of <UUID>_1_201_a.jpg` = a mixed batch that INCLUDES the main-37 verified SKUs and is NOT filename-mapped. For a main-37 SKU there is no filename hint → **VIEW the candidate images (Read tool) to match product↔photo; NEVER guess-copy** (wrong toy = wrong render). Confirmed match: Wooden World Flags Puzzle `4860129139007` = `Copy of 4A597A2A-FA1E-4A68-8676-A4A0B516CB37_1_201_a.jpg`. Every `.md` SPEC inside references the SKU photo as `sku_ref.jpg` (or `./sku_ref.jpg`), NEVER the absolute source path with spaces / `Copy of …` prefix. Render operator drag-drops three files into Nano Banana 2: `agents/bubabu/1.jpeg` + `agents/bubabu/2.jpeg` + `[pair-folder]/sku_ref.jpg`. Multi-SKU bundles use `sku_ref_1.jpg` / `sku_ref_2.jpg`. Full law: `memory/feedback_bubabu_sku_ref_local_copy_in_pair_folder.md`. Pre-save grep gate: zero hits on `Copy of IMG_` / `BUBABU.New Products` inside the pair-folder.
...
| **Visual style** | **ULTRA-DETAILED Pixar feature-film 3D FINAL-RENDER quality (quality-rich anchor 2026-06-01)** - Disney/Pixar studio polish grade, sharp clean geometry, crisp edge separation, high-frequency detail on plush fur + material textures, vivid saturated Candy Pop kid-toy palette (NOT muted/washed out/desaturated), bright clear studio lighting (warm key + soft fill + gentle rim). Quality bar refs: Boo close-up from Monsters Inc + Carl Fredricksen warmth from Up + Miguel detail from Coco. Per-ep variant palette declared from SKIN_VARIANTS grid below. NEVER muddy, NEVER soft-focused, NEVER cheap-3D. NEVER photoreal tokens (no Sony/Sigma/DSLR/ISO). NEVER watercolor / hand-painted / illustrated / 2D anime / paper-grain / ink-contour. NEVER gold-leaf / white-gold / peach-pink / lavender / pure-white-glow / vertical light-shafts. |
