...
CHARACTER_BLOCK:
```
STATIC: Pixar stylized 3D young boy, Dash (Incredibles) + Miguel (Coco) DNA — large bright Pixar
        eyes, short spiky dark hair, round cheeks, button nose, soft Pixar 3D skin shader (NOT
        photoreal), toy-like proportions, small stature, energetic builder.
        ANCHORS: short spiky dark hair · a red-and-white striped tee · denim dungarees.
DYNAMIC: red-and-white striped tee under denim dungarees, rolled cuffs, white socks, blue sneakers.
[expression: ...]  ← per-shot only (focused / determined / delighted / proud).
```
- bone `delicate` · face_id 0.9 / locked. persistent_features (paste exactly, zero real-anatomy):
```
"Pixar stylized 3D young boy, Dash + Miguel DNA, large bright Pixar eyes"
"short spiky dark hair, red-and-white striped tee"
"denim dungarees, soft Pixar 3D skin shader not photoreal, toy-like proportions"
```
- Zero child speech (narrator-solo). Mouth sealed closed every scene.

## 3 · WONDER-BRAIN (reaction PROP)
```
...
```
no glow on Bubabu body, no glow on goggles, no open beak, no 3.jpeg heart-eyes variant, no second Bubabu,
no scary owl, no realistic owl, no brown ears, no caramel body,
no tail, no tail feathers, no bird tail, perfectly round spherical ball body, no body elongation, no stretching, no squash-and-stretch, Bubabu rigid round and frontal,
no photoreal, no live-action, no DSLR, no real-skin SSS, no film grain, no HDR cinema, no soft haze, no dust-motes, no atmospheric blur,
no realistic child, no realistic human anatomy, no child in bed, no pyjamas, no sleeping child,
no clinical brain, no anatomical brain, no medical imagery,
no second car, no duplicate car, no extra toy, no screen, no phone, no tablet,
text artifacts, watermark, deformed hands, extra fingers, blurry, low quality
```

## CHE$9 self-check
- [x] Bubabu reference-only + FRONTAL+STATIC+no-tail (fidelity fix) across SPECs
- [x] NEW boy byte-locked, distinct from ep2; ≥3 anchors each
- [x] No personality words in STATIC/DYNAMIC; no names in prompts; mouth sealed/awake/daytime
- [x] Wonder-brain = reaction PROP (SNAP on, not steady glow); glow on prop only
- [x] SKU reference-only (no brand/colors); style anchor = BRIGHT GLOSSY not arthouse
