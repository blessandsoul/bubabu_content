# COVER — Bubabu Brain Remedy · Magnetic Blocks (15 days)
...
> Frmt mirrors the Remedy locked cover formula — `[noun] 15 დღე` + `რა ხდება [...]?` — retargeted from body to a child's brain.
...
## DESIGN CONCEPT
Pixar feature-film candy-pop poster. Bubabu hero center-frame in **curious wonder** (Remedy face-map: curious wonder / quiet awe — the viewer wants to discover too), beak closed, leaning over an open box of bright magnetic blocks with one held up to his goggles like a tiny window of light. A faint cute wonder-brain bubble glows soft above his shoulder — just enough to ask the question, never the finished build (mid-action, never the result). Butter-cream sunburst behind, candy confetti. The whole frame says *"a toy is waking something up"* in 0.3s.

**Palette (60-20-10, FB-blue-UI contrast):** 60% warm butter-cream `#FFFAEB→#FFF6CC` · 20% coral→magenta focal (`#FF6B6B`/`#FF1F8F`) on the headline + confetti · 10% cyan `#5BC0DE` Bubabu-goggle accent + sub. Warm-saturated, zero blue/navy dominant (pops against Facebook's blue chrome).

## CENTRAL HEADLINE (baked, Mkhedruli lowercase)
- **Main headline:** `კუბიკები 15 დღე` — *"Blocks, 15 days"* — **NUMBER anchor = 15** (EPIC mandate satisfied). Two-word + number, chunky rounded.
- **Subheadline (curiosity gap, SERIES-LOCKED 2026-06-05):** `რა ხდება შენი ბავშვის ტვინში?` — *"what happens in YOUR child's brain?"* — **second-person upgrade** (replaces third-person "a child's"): targets the watching parent + fires the "your child" > "children" share-trigger, semantically correct. The image shows WHAT (toy + glowing wonder-brain), the title asks WHAT-INSIDE. Complement, not duplicate. **KA user-verify; identical on every episode cover (ep2 + ep3 matched).**
...
**EPIC anchor declared:** NUMBER (`15`) + concrete noun pair (`კუბიკები` blocks / `ტვინი` brain) + curiosity question. Zero banned slop intensifiers.
...
**Refs uploaded:** `agents/bubabu/1.jpeg` + `agents/bubabu/2.jpeg` (Bubabu form, NEVER `3.jpeg`) + `sku_ref.jpg` (Magnetic Block 36pc — reference-only, appearance never described).
...
5. Per-glyph Mkhedruli render check: კ-უ-ბ-ი-კ-ე-ბ-ი / 1-5 / დ-ღ-ე / რ-ა / ხ-დ-ე-ბ-ა / ბ-ა-ვ-შ-ვ-ი-ს / ტ-ვ-ი-ნ-შ-ი.

## TYPOGRAPHY HIERARCHY
| Layer | Text | Treatment | Color | Function |
|---|---|---|---|---|
| Headline | `კუბიკები 15 დღე` | chunky rounded + GRADIENT FILL + soft shadow | coral→magenta | promise + number hook |
...
| 2 | Mid-action | ✅ Bubabu curious over the blocks + faint brain glow — the moment BEFORE the build, never the finished result |
| 3 | Curiosity gap | ✅ image shows toy+glow, title asks "what happens in the brain?" — complement not duplicate |
| 4 | Color contrast vs FB blue UI | ✅ warm butter-cream + coral→magenta headline, zero blue/navy dominant |
| 5 | One focal ≤3 elements | ✅ Bubabu face = focal; blocks + brain-glow secondary (Z-pattern) |
...
## A/B/C/D VARIANTS
- **B — HANDS-FIRST:** close on the child's small hands holding one glowing-edged tile, Bubabu soft-focus behind, same headline. Switch if A's mascot-poster CTR < FB 1.5% after 48h.
- **C — SPLIT BRAIN:** left half a dim sleepy wonder-brain, right half the same brain bright and glowing, blocks between, headline across. Gravitas / clearer before-after. Try if curiosity-gap underperforms.
- **D — BUBABU CLOSE-UP:** 70% face crop of Bubabu in pure wonder, one tile reflected in the goggle-fabric (no glow), tiny brain bubble corner. Pull only if A+B+C flat — strongest emotional-face option.
